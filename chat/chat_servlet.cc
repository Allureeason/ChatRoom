...
void session_notify(ChatMessage::ptr msg, sylar::http::WSSession::ptr session = nullptr) {
    ...
    for(auto& i : sessions) {
        ...
        SYLAR_LOG_INFO(g_logger) << "notify " << i.first << " msg:" << msg->toString();
...
int32_t ChatServlet::onConnect(sylar::http::HttpRequest::ptr header
                              ,sylar::http::WSSession::ptr session) {
    SYLAR_LOG_INFO(g_logger) << "onConnect " << session;
    return 0;
}

int32_t ChatServlet::onClose(sylar::http::HttpRequest::ptr header
                             ,sylar::http::WSSession::ptr session) {
...
int32_t ChatServlet::handle(sylar::http::HttpRequest::ptr header
